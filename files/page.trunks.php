    ...
    <tr><td colspan="2">
      ...
      <input type="button" id="dial-pattern-clear"  value="<?php echo _("Clear all Fields")?>" />
			...
			<tr>
				...
				</td><td valign="top"><select id="autopop"  tabindex="<?php echo ++$tabindex;?>" name="autopop" onChange="insertCode(); ">
						<option value="" SELECTED><?php echo _("(pick one)")?></option>
						<option value="n10n11"><?php echo _("N10 N11 (Alterar prepend ZX para o DDD)")?></option>
						<option value="falevox"><?php echo _("Falevox Trunk (Alterar prefix ZX para o DDD)")?></option>
...
function insertCode() {
        ...
        if ($('#pattern_file').val() == '') {
          $('#pattern_file').hide();
				...
				code = document.getElementById('autopop').value;
				insert = '';
				switch(code) {
					case "local":
	    insert = '<?php echo _(",,NXXXXXX") ?>';
					...
					case "local10":
	    insert = '<?php echo _(",,NXXXXXX") ?>';
	    insert += '<?php echo _("/,,NXXNXXXXXX") ?>';
					break;
					case 'tollfree':
	    insert += '<?php echo _(",,1800NXXXXXX") ?>';
            insert += '<?php echo _("/,,1888NXXXXXX") ?>';
            insert += '<?php echo _("/,,1877NXXXXXX") ?>';
            insert += '<?php echo _("/,,1866NXXXXXX") ?>';
            insert += '<?php echo _("/,,1855NXXXXXX") ?>';
            insert += '<?php echo _("/,,1844NXXXXXX") ?>';
					break;
					case "ld":
            ...
            insert = '<?php echo _(",,011.") ?>';
					break;
					case 'info':
            insert = '<?php echo _(",,411") ?>';
            insert += '<?php echo _("/,,311") ?>';
					break;
					case 'emerg':
            insert = '<?php echo _(",,911") ?>';
					break;
					case 'n10n11':
	    insert = '<?php echo _("ZX,,9XXXXXXX") ?>';
		insert += '<?php echo _("/ZX,,9XXXXXXXX") ?>';
		insert += '<?php echo _("/,,ZX[2-5]XXXXXXX") ?>';
		insert += '<?php echo _("/,,ZX9XXXXXXXX") ?>';
		insert += '<?php echo _("/ZX,0,9XXXXXXX") ?>';
		insert += '<?php echo _("/ZX,0,9XXXXXXXX") ?>';
		insert += '<?php echo _("/,0,ZX[2-5]XXXXXXX") ?>';
		insert += '<?php echo _("/,0,ZX9XXXXXXXX") ?>';
		insert += '<?php echo _("/,0ZX,ZX[2-5]XXXXXXX") ?>';
		insert += '<?php echo _("/,0ZX,ZX9XXXXXXXX") ?>';
		insert += '<?php echo _("/,55,ZX[2-5]XXXXXXX") ?>';
	    insert += '<?php echo _("/,55,ZX9XXXXXXXX") ?>';
					break; 
					case 'falevox':
	    insert = '<?php echo _("55,,ZX9XXXXXXXX") ?>';
		insert += '<?php echo _("/55,,ZX9XXXXXXX") ?>';
		insert += '<?php echo _("/55,,ZX[2-5]XXXXXXX") ?>';
		insert += '<?php echo _("/,ZX,9XXXXXXXX") ?>';
		insert += '<?php echo _("/,ZX,9XXXXXXX") ?>';
		insert += '<?php echo _("/,ZX,[2-5]XXXXXXX") ?>';
	...
	for(let i=0; (i<patterns.length); i++){
		...
		var prepend = fields[0];
        	var prefix = fields[1];
        	var match = fields[2];

		addCustomField(prepend, prefix, match, $("#last_row"));
	}
